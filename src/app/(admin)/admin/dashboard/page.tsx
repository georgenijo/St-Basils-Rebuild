  ...
  return (
    <main className="px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section aria-label="Admin features">
        <h2 className="font-heading text-xl font-semibold text-wood-900">Manage</h2>
          ...
          {placeholderCards.map((card) => (
            <Link key={card.title} href={card.href}>
              <Card variant="outlined" className="p-6 transition-shadow hover:shadow-md">
                ...
                <h3 className="font-heading text-lg font-semibold text-wood-900">{card.title}</h3>
                <p className="mt-1 text-sm text-wood-800/60">{card.description}</p>
              </Card>
            </Link>
          ))}
        </div>
